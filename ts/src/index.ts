...
const films: TFilm[] = [
    ...
    },
    {
        ...
        writer: "Steve Kloves, J.K. Rowling",
        actors: ["Daniel Radcliffe", "Emma Watson", "Rupert Grint"],
        plot: "As Harry Potter begins his sixth year at Hogwarts, he discovers an old book marked as 'the property of theHalf-Blood Prince' and begins to learn more about Lord Voldemort\'sdark past.",
...
const allActors = films.map(film => film.actors)
    .reduce((acc, actors) => {
...
const sortedByRating = films.sort((a, b) => b.imdbRating - a.imdbRating);

const simplifiedFilms = films.map(({id, title, released, plot}) => ({id, title, released, plot}));

const filterByYear = (films: TFilm[], year: number) => films.filter(film => film.year === year);

const filterByTitle = (films: TFilm[], title: string) => films.filter(film => film.title.toLowerCase().includes(title.toLowerCase()));
...
const filteredByTitleOrPlot = filterByTitleOrPlot(films, 'action');
const filteredByParam = filterFilmsByParam(films, 'year', 2011);
